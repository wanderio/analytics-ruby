      ...
      describe Worker do
        describe "#init" do
          it 'accepts string keys' do
            queue = Queue.new
            worker = Tracking::Segment::Analytics::Worker.new(queue, 'secret', 'batch_size' => 100)
        ...
        describe '#run' do
          before :all do
            Tracking::Segment::Analytics::Defaults::Request::BACKOFF = 0.1
          end

          after :all do
            Tracking::Segment::Analytics::Defaults::Request::BACKOFF = 30.0
          end
          ...
          it 'does not error if the endpoint is unreachable' do
            expect do
              ...
              queue = Queue.new
              queue << {}
              worker = Tracking::Segment::Analytics::Worker.new(queue, 'secret')
              worker.run
          ...
          it 'executes the error handler, before the request phase ends, if the request is invalid' do
            Tracking::Segment::Analytics::Request.any_instance.stub(:post).and_return(Tracking::Segment::Analytics::Response.new(400, "Some error"))
            ...
            queue = Queue.new
            queue << {}
            worker = Tracking::Segment::Analytics::Worker.new queue, 'secret', :on_error => on_error
            ...
            Tracking::Segment::Analytics::Request::any_instance.unstub(:post)
          ...
          it 'does not call on_error if the request is good' do
            ...
            worker = Tracking::Segment::Analytics::Worker.new queue, 'testsecret', :on_error => on_error
            worker.run
        ...
        describe '#is_requesting?' do
          it 'does not return true if there isn\'t a current batch' do
            queue = Queue.new
            worker = Tracking::Segment::Analytics::Worker.new(queue, 'testsecret')
